...
Метод позволяет создать новый поток `flow`. При создании `flow` с типом `chat` в ключе `users` необходимо указать uuid двух пользователей. При создании `flow` с любым типом ключ `owner` должен содержать uuid владельца этого `flow`.
