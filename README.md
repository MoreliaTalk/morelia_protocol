...
Актуально на: **25.03.2022**

Версия протокола: **2.0** Редакция протокола: **1**
...
Поле предназначено для передачи информации об используемых протоколах шифрования.
...
Event служит для передачи от клиента серверу дополнительного запроса на получение обновлений данных (сообщений, потоков и пр.).
Такой запрос сервер обрабатывает во время сессии websocket-соединения.
...
| time_start | int       | No / No                                      | Время с которого сервер должен отправить обновление.                        |
...
- workspace (мета-поток для объединения в единое пространство потоков chat, channel, group)

| Ключ          | Тип       | Обязательный (запрос / ответ) | Описание                                                                                                                   |
|---------------|-----------|-------------------------------|----------------------------------------------------------------------------------------------------------------------------|
| uuid          | str       | Yes / Yes                     | Уникальный номер потока.                                                                                                   |
| time_created  | int       | No / No                       | Время создания потока, в секундах со времени начала эпохи (Unix-время).                                                    |
| type          | str       | No / No                       | Тип потока (chat, channel, group, workspace)                                                                               |
...
| sub_flow      | list[int] | No / No                       | Список uuid потоков `flow` входящих в Workspace. Для chat, channel, group значенеи поля пустое.                            |


### Поле message

В `message` передается информация о сообщении. Если у сообщения есть ветка `thread` то это сообщение является корневым.
...
| time_created  | int                      | No / No                       | Время когда сообщение было написано, в секундах со времени начала эпохи (Unix-время).                                |
...
| threads       | list[str]                | No / No                       | Uuid message которые относятся к thread.                                                                             |
| content       | [content](#поле-content) |                               | Содержание сообщения.                                                                                                |
...
| info           | [info](#поле-info)                     | Yes / Yes                     | Информация о контенте.                                             |
| thumbnail_info | [thumbnail_info](#поле-thumbnail_info) | No / No                       | Превью (для изображения или видео).                                |
...
Мета-информация о контенте, а также ссылка на файл.
...
Мета-информация о превью (только для изображения и видео), а также ссылка на файл.
...
| hash_password | str       | No / No                       | Хэш от хэша пароля пользователя.                                    |
...
| flow_invite   | list[int] | No / No                       | Потоки в которые пользователь был приглашен.                        |
...
Коды ошибок (и их статусы) соответствуют кодам протокола [HTTP](https://ru.wikipedia.org/wiki/%D0%A1%D0%BF%D0%B8%D1%81%D0%BE%D0%BA_%D0%BA%D0%BE%D0%B4%D0%BE%D0%B2_%D1%81%D0%BE%D1%81%D1%82%D0%BE%D1%8F%D0%BD%D0%B8%D1%8F_HTTP).
Значения ключа `detail` предназначено для подробного разъяснения статуса выполнения запроса.
...
### Метод get_update_flow

Позволяет получить от сервера доступную информацию о flow, от времени time по настоящее время. 
...
### Метод get_update_message

Позволяет получить от сервера доступную информацию о сообщениях которые принадлежать определенному flow, от времени time по настоящее время.
...
### Метод get_update_user

Позволяет получить от сервера доступную информацию о пользователях, от времени time по настоящее время.
...
Сервер в ответ отправляет клиенту новый сессионный ключ подписанный и зашифрованный открытым ключем клиента, в ответе статус "Accepted" и код "202"
