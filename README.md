...
Запросы между клиентом и сервером передаются в виде JSON-объекта. В каждом запросе имеется ключ `type` значение которого является именем метода. Запрос в котором указан неподдерживаемый метод всегда вернёт ошибку _400 Bad Request_.
...
Содержание JSON-объекта, содержащее все используемые поля.
