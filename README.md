...
Ключ | Тип | Обязательный | Описание
---- | --- | ------------ | --------
uuid | str | No | Уникальный номер пользователя. Выдается сервером после аутентификации.
...
Имя метода передается как значение ключа `type` внутри объекта сериализованного как строка с JSON-форматированием.
