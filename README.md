...
from_user_uuid | str | No | Уникальный номер пользователя который написал сообщение.
...
Метод позволяет создать новый поток `flow`. При создании `flow` с типом `chat` в запросе необходимо указать uuid для двух пользователей.
...
```javascript
{
    ...
    "data": {
        ...
        "user": [{
            ...
            {
            "uuid": "548r4578-ty67-1189-g1e2-495858720400"
            }],
        "meta": null
        },
    "jsonapi": {
